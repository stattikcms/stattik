from botworx.compile.compilerbase import CompilerBase
#from botworx.compile.policies.default import DefaultPolicy
...
'''
    ...
    def QClause(self, node):
        ...
        c = node.expr
        header = "$query = self.rnr.ctx.query(" if node.first else ".and("
    ...
    def QNegClause(self, node):
        ...
        c = node.expr
        header =  "$query = self.rnr.ctx.query(" if node.first else ".not("
        return self.writeLn(''.join([
            header,
            ''.join([
                self.visit(c.type),
